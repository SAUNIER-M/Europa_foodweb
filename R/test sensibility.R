...
# Predation autres
prCoPBJ <- 2; prRPBJ <- 5; prCHPBJ <- 2; prCHABJ <- 0.01
prCoPBR <- 5; prRPBR <- 10; prCHPBR <- 5; prCHABR <- 0
prRPCH <- 0
...
europa_fct_safe <- function(t, y, parms){
  ...
  with(as.list(parms), {
    ...
    # Barn owls
    dYCH <- max(aCH*GCH*ACH*(1-ACH/KCH),eps) - phiYCH*YCH - muYCH*YCH - prRPCH*R*(YCH/Proies_R)
...
# -----------------------------
# 4️⃣ Boucle sur tous les paramètres simples
# -----------------------------
param_list <- list(
  SRCH=seq(0,1,0.01),
  GCH=seq(1,8,1),
...
for(param_name in names(param_list)){
  ...
  results <- data.frame(val=vals, lambda_CH=NA, lambda_PBJ=NA, lambda_PBR=NA, lambda_R=NA, lambda_Co=NA)
  ...
  for(i in seq_along(vals)){
    ...
    # Calculer lambda pour toutes les populations
    results$lambda_CH[i] <- calc_lambda(out$time, out$YCH + out$JCH1 + out$JCH2 + out$ACH)
    results$lambda_PBJ[i] <- calc_lambda(out$time, out$YBJ + out$JBJ1 + out$JBJ2 + out$JBJ3 + out$JBJ4 + out$JBJ5 + out$JBJ6 + out$ABJ)
    results$lambda_PBR[i] <- calc_lambda(out$time, out$YBR + out$JBR1 + out$JBR2 + out$JBR3 + out$JBR4 + out$JBR5 + out$JBR6 + out$ABR)
  ...
  p <- ggplot(results, aes(x=val,y=lambda_CH)) +
    geom_line() + geom_point() +
    geom_hline(yintercept=1, linetype="dashed",color="red") +
    labs(x=param_name, y="λ Chouettes",
         title=paste("Sensibilité λ chouettes selon", param_name)) +
    theme_bw()
  ggsave(here("output",paste0("lambda_CH_",param_name,".png")), p, width=8, height=6, dpi=300)
}

# -----------------------------
# 5️⃣ Sensibilité combinée roCo × roR (isolignes) pour PBJ et PBR
# -----------------------------
roCo_vals <- seq(0,1,0.1)
...
for(i in 1:nrow(results_comb)){
  parms <- list(
    ...
    rR=results_comb$roR[i], KR=KR, roR=results_comb$roR[i],
...
# Graphique isolignes PBJ
p_PBJ_contour <- ggplot(results_comb, aes(x=roCo, y=roR, z=lambda_PBJ)) +
  geom_contour(color="black") +
  geom_text(stat="contour", aes(label=..level..), size=3) +
  labs(x="roCo (corbeaux pies)", y="roR (rats)", title="Isolignes λ PBJ") +
  theme_minimal()
ggsave(here("output","lambda_PBJ_roR_roCo_contours.png"), p_PBJ_contour, width=8, height=6, dpi=300)

# Graphique isolignes PBR
p_PBR_contour <- ggplot(results_comb, aes(x=roCo, y=roR, z=lambda_PBR)) +
  geom_contour(color="black") +
  geom_text(stat="contour", aes(label=..level..), size=3) +
  labs(x="roCo (corbeaux pies)", y="roR (rats)", title="Isolignes λ PBR") +
  theme_minimal()
ggsave(here("output","lambda_PBR_roR_roCo_contours.png"), p_PBR_contour, width=8, height=6, dpi=300)
